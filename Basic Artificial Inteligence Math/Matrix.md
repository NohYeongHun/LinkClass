...
# 정칙행렬(가역행렬-invertible matrix)
1. 정의 :<br>
n차 정사각행렬 A에 대하여 다음을 만족하는 행렬 B가 존재하면 A는 가역행렬 또는 정칙행렬이라고 함.<br>
AB = (n차)I / BA = (n차)I
이때 B를 A의 역행렬(inverse matrix)이라고 하며, 이러한 B가 존재하지 않으면 A는 비가역(nonivertible)또는 비정칙(singular)행렬이라고 한다. 일반적으로 A의 역행렬을 A^(-1)로 나타낸다. 가역행렬의 역행렬은 유일하다.
2. 성질
- 크기가 같은 두 정사각행렬 A,B가 가역행렬.
- A도 가역행렬, (A^-1)-1 = A
- kA도 가역행렬 (kA)^-1 = 1/k*A^-1
- AB도 가역행렬 (AB)^-1 = B^-1*A^-1
- A^n도 가역행렬 (A^n)^-1 = (A^-1)^n
- (A^T=A의 전치행렬)도 가역행렬 (A^T)^-1 = (A^-1)^T

# 선형연립방정식
- 미지수의 개수와 방정식의 개수가 같은 선형연립방정식
- Ax = b
- 선형연립방정식에서 n차 정사각행렬 A =a[ij]가 가역행렬이면 다음과 같이 유일한 해를 갖는다.
x = (A^-1*b)
- A가 가역행렬임을 어떻게 판단할까?
- A가 가역행렬일 때, 역행렬 A^-1은 어떻게 구할까?
- 단위행렬 I(n차)에 기본행연산을 한번 적용해서 얻어진 행렬을 기본행렬이라 한다.
- 기본행렬의 성질 : <br>
![](2021-03-29-11-28-33.png)
정사각행렬의 왼쪽에 기본행렬을 곱한 결과는 기본행렬에 대응하는 기본행연산을 주어진 행렬에 시행한 결과와 같다.
![](2021-03-29-11-29-10.png)
```Math
A = matrix(3,3,[1,2,3,1,1,1,0,1,3])
A의 R2행과 R3행의 위치를 바꾼다.
B는 R2행과 R3행의 위치를 바꾸는 연산행렬
...
- 임의의 n차 정사각행렬 A에 대하여 다음은 동치이다.
- A는 가역 행렬이다.
- A는 I(n차)와 행동치이다. (즉,RREF(A)=I(n차)
)
- A는 기본행렬들의 곱으로 쓸 수 있다.
- Ax = 0은 자명한 해 0을 가진다.
- Ax = b는 모든 b에 대해 유일한 해를 갖는다.
- n차 정사각행렬 A가 가역행렬일 필요충분조건은 det(A) = |A|!=0이다.
- 2*2 행렬 A=[a b][c d]가 가역행렬일 필요충분조건은 ad-bc!=0이다.